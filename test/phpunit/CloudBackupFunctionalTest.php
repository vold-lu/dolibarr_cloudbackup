 ...
 * Copyright (C) 2022 Aloïs Micard <a.micard@vold.lu>
 *
...
/**
 * \file    test/phpunit/CloudBackupFunctionalTest.php
 * \ingroup cloudbackup
...
/**
 * Class CloudBackupFunctionalTest
 *
 ...
 *
 * @package Testcloudbackup
 */
class CloudBackupFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modCloudBackup')->click();
		} else {
			// Disable the module
			$this->byHref('modCloudBackup')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/cloudbackup/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('cloudbackup/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/cloudbackup/admin/about.php');
		$this->authenticate();
		return $this->assertContains('cloudbackup/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('cloudbackupwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modCloudBackup_CloudBackupTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modCloudBackup_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
